...
{ 
    return getPromise( [ ...validUsers ] as types.Users );  
}

export function getUsersForTable( page: number, rowAmount: number ) : Promise<types.Users>
{ 
    if ( rowAmount === -1 )
    { 
        return getPromise( [ ...validUsers ] as types.Users ); 
    ...
    for (let i = 0; i < validUsers.length; i += rowAmount) 
    {
        const chunk = validUsers.slice(i, i + rowAmount);
        
        slices.push( chunk );        
    }

    return getPromise( [ ...slices[ page ] ] as types.Users );  
}

export function getUsersCount( ) : Promise<number>
{ 
    return getPromise( validUsers.length as number );  
}
...
{ 
    ...
    if ( validUsers.findIndex( user => user.userName === updatedNewUser.userName ) === (-1) )
    {
